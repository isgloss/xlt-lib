...
function generateFromEntropy(entropy, version, testSecret) {
    ...
    var payload = (addressVersion == 2 ? String.fromCharCode(0) : '') + account_id
    ...
    var secret = Base58.encode(strToArray(String.fromCharCode(version) + arrayToStr(entropy)))
...
class hmac_drbg {
    constructor(entropyInput, personalizationString) {
        ...
        this._ENTROPY_INPUT_SIZE_BYTES = Math.floor(((this._SECURITY_STRENGTH / 8) * 3) / 2)
        ...
        this._value = ''
        for (var i = 0; i < this._DIGEST_NUM_BYTES; i++) this._value += String.fromCharCode(1)
    ...
    nextBytes2(out, start, count) {
        ...
        if (this._bytesGenerated + count > this._MAX_BYTES_TOTAL) {
            throw new Error('Cannot generate more than a total of ' + count + ' bytes.')
        }
        try {
            var bytesWritten = 0
            while (bytesWritten < count) {
                var bytesToWrite = Math.min(count - bytesWritten, this._MAX_BYTES_PER_REQUEST)
...
const createXLTTransaction = (address, to_address, amount, comment) => {
    return {
        address,
        to_address,
        val: parseFloat(amount).toFixed(6),
        comment: comment || '',
    }
}
const signXLTTransaction = (tx, publicKey, privateKey) => {
    return {
        ...tx,
        sign: signStr(`{"address":"${tx.address}","to_address":"${tx.to_address}","val":${tx.val}}`, publicKey, privateKey),
    }
}
module.exports = {
